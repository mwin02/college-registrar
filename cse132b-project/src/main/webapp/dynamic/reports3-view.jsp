...
<head>
<meta charset="UTF-8">
<title>Queries : Reports 3 - Materialized Views</title>
<style>
table, th, td {
...
<body>
	...
	<h1>Reports Three - Materialized Views</h1>
	<% 
...
/* 			String GPA_REPORT_QUERY = "SELECT avg(gc.NUMBER_GRADE) as gpa FROM Class class, enrollment e, GRADE_CONVERSION gc " +
			...
			String GPA_REPORT_QUERY = "";
			if (!curQuarter.equals("%")){
				GPA_REPORT_QUERY = "select grade, grade_count from cpqg where quarter = '" + curQuarter + "' AND faculty = '" + curFaculty + "' AND course_id = " + curCourseID + " ORDER BY grade";
			} else {
				GPA_REPORT_QUERY = "select grade, grade_count from cpg where faculty = '" + curFaculty + "' AND course_id = " + curCourseID + " ORDER BY grade";
			}
	...
	<section>
		...
		<form action="reports3-view.jsp" method="post">
		...
		<h3>Course ID: <%= curCourseID %> &emsp; Quarter: <%= (curQuarter.equals("%")) ? "Any" : curQuarter%>&emsp;
			Faculty: <%= (curFaculty.equals("%")) ? "Any" : curFaculty %> &emsp;
		</h3>
		<table>
			<tr>
				<th> Letter </th>
	...
	%>
		...
		<p> <%= e %> </p>]
		<a href="reports3-view.jsp">Back To Reports Page</a>
	<% 
	} finally {
		if (rs != null) {
			rs.close();			
